...
export class InputValidator {
    constructor() {
        ...
        this.errorIcons = new Map();
    }
    /**
     * 입력 필드에 실시간 검증 적용
     */
    setupValidation(input, config) {
        // ARIA 속성 초기 설정
     ...
     * ARIA 속성 설정
     */
    setupAriaAttributes(input, config) {
        // aria-required 설정 (필수 규칙이 있는 경우)
        const hasRequired = config.rules.some(rule => rule.errorMessage.includes('입력해주세요') ||
     ...
     * 에러 상태 표시
     */
    showError(input, message, config) {
        // 테두리 하이라이트
...
export class ValidationRules {
     ...
     * 숫자 범위 검증 규칙
     */
    static range(min, max, fieldName) {
        return {
            validate: (value) => {

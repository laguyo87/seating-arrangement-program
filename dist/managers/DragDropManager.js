...
export class DragDropManager {
    constructor(seatsAreaId, onDrop, isFixedSeat) {
    ...
    enable() {
        ...
        this.seatsArea.addEventListener('drop', (ev) => {
            ...
            if (!targetCard && this.seatsArea) {
                ...
                const allCards = Array.from(this.seatsArea.querySelectorAll('.student-seat-card'));
    ...
    setupTouchEvents() {
        ...
        this.seatsArea.addEventListener('touchend', (e) => {
            ...
            if (this.isDragging) {
                ...
                let targetCard = null;
                if (elementBelow) {
                    const card = elementBelow?.closest('.student-seat-card');
     ...
     * 터치 드래그 오버 상태 업데이트
     */
    updateDragOverStateForTouch(targetCard) {
        if (!this.seatsArea)
            return;
        // 기존 하이라이트 제거
    ...
    updateDragOverState(e) {
        if (!this.seatsArea || !this.dragSourceCard)
            return;
        // 기존 하이라이트 및 인디케이터 제거
        this.cleanupDragState();

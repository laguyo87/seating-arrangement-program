...
export class CSVFileHandler {
     ...
     * CSV 파일 읽기
     */
    readCsvFile(file, updateProgress) {
     ...
     * 엑셀 파일 읽기
     */
    readExcelFile(file, updateProgress) {
        const reader = new FileReader();
        if (updateProgress) {
     ...
     * 미리보기 및 확인 다이얼로그 표시
     */
    showPreviewAndConfirm(students, fileName) {
        // 미리보기 모달 생성
     ...
     */
    createTableWithStudentsAsync(students) {
        const chunkSize = 50;
        let currentIndex = 0;
        const processChunk = () => {
     ...
     */
    appendStudentsToTable(students) {
     ...
     * 학생 테이블 행 생성
     */
    createStudentTableRow(student, globalIndex, tbody) {
     ...
     * 저장된 학생 데이터를 테이블에 로드
     */
    loadStudentDataToTable(studentData, retryCount = 0) {
        const MAX_RETRIES = 5;
        const outputSection = document.getElementById('output-section');
        if (!outputSection) {
...
//# sourceMappingURL=CSVFileHandler.js.map
...
export class RandomService {
     ...
     * @returns 셔플된 학생 배열
     */
    static shuffleStudents(students) {
     ...
     * @returns 배치된 좌석 배열
     */
    static assignRandomly(students, seats) {
        // 사용 가능한 좌석만 필터링
     ...
     * @returns 배치된 좌석 배열
     */
    static assignGenderPairs(students, seats, seatType) {
        ...
        if (seatType === 'pair') {
            ...
            for (const pair of shuffledPairs) {
                if (seatIndex >= availableSeats.length - 1)
                    break;
                // 짝꿍 좌석에 배치
     ...
     * @returns 배치된 좌석 배열
     */
    static assignWithFixedSeats(students, seats) {
     ...
     */
    static reshuffleAssignments(seats) {
        // 기존 배정 정보를 먼저 저장
        const assignments = [];
        ...
        for (let i = 0; i < minLength; i++) {
            SeatModel.assignStudent(availableSeats[i], shuffledAssignments[i].studentId, shuffledAssignments[i].studentName);
        }
    }
}

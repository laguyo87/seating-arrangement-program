...
import { LoginPageModule } from '../modules/LoginPageModule.js';
import { SignUpPageModule } from '../modules/SignUpPageModule.js';
...
export class MainController {
    constructor() {
        ...
        this.isReadOnlyMode = false; // 읽기 전용 모드 (이력에서 불러온 경우)
        // 메모리 누수 방지를 위한 추적 변수
        this.eventListeners = [];
        ...
        try {
            ...
            const visitorCounterModuleDeps = {
                firebaseStorageManager: this.firebaseStorageManager
            };
     ...
     */
    toggleAvoidPrevPartnerOption(enable) {
        const avoidPrevPartner = document.getElementById('avoid-prev-partner');
        if (avoidPrevPartner) {
            if (enable) {
                // 활성화
    ...
    handleConfirmSeats() {
        try {
            ...
            const container = this.outputModule.container;
            if (container) {
                // 기존 메시지 제거
     ...
     */
    updateSeatsAndStudentsFromLayout(currentLayout) {
        try {
    ...
    async loadHistoryItem(historyId) {
        try {
            ...
            const femaleInput = document.getElementById('female-students');
            if (maleInput)
                maleInput.value = maleCount.toString();
            if (femaleInput)
                femaleInput.value = femaleCount.toString();
            ...
            if (historyItem.layoutType) {
                ...
                // 먼저 모든 라디오 버튼의 체크 해제
                document.querySelectorAll('input[name="layout-type"]').forEach((input) => {
                    input.checked = false;
                });
                // layout-type 라디오 버튼 설정 (이벤트 발생 없이)
                const layoutTypeRadio = document.querySelector(`input[name="layout-type"][value="${historyItem.layoutType}"]`);
                if (layoutTypeRadio) {
                ...
                if (historyItem.layoutType === 'single-uniform') {
                    ...
                    if (historyItem.singleMode) {
                        document.querySelectorAll('input[name="single-mode"]').forEach((input) => {
                            input.checked = false;
                        });
                        const singleModeRadio = document.querySelector(`input[name="single-mode"][value="${historyItem.singleMode}"]`);
                        if (singleModeRadio) {
                    ...
                    if (historyItem.pairMode) {
                        document.querySelectorAll('input[name="pair-mode"]').forEach((input) => {
                            input.checked = false;
                        });
                        const pairModeRadio = document.querySelector(`input[name="pair-mode"][value="${historyItem.pairMode}"]`);
                        if (pairModeRadio) {
                    ...
                    if (historyItem.groupSize) {
                        document.querySelectorAll('input[name="group-size"]').forEach((input) => {
                            input.checked = false;
                        });
                        const groupSizeRadio = document.querySelector(`input[name="group-size"][value="${historyItem.groupSize}"]`);
                        if (groupSizeRadio) {
                ...
                // 복원된 옵션 확인 및 검증
                const restoredLayoutType = document.querySelector('input[name="layout-type"]:checked');
                const actualLayoutType = restoredLayoutType?.value;
                // pair-uniform인 경우 pair-mode도 확인
                let actualPairMode = null;
                if (historyItem.layoutType === 'pair-uniform') {
                    const restoredPairMode = document.querySelector('input[name="pair-mode"]:checked');
            ...
            // 모든 카드 초기화
            let allCards = Array.from(seatsArea.querySelectorAll('.student-seat-card'));
            // 좌석 카드가 없거나 개수가 맞지 않으면 카드 생성
            if (allCards.length === 0 || allCards.length < totalSeats) {
                // 옵션이 제대로 복원되었는지 다시 한 번 확인
                const verifyLayoutType = document.querySelector('input[name="layout-type"]:checked');
                ...
                if (historyItem.layoutType === 'pair-uniform') {
                    ...
                    for (let row = 0; row < rowsPerPartition; row++) {
                        for (let partition = 0; partition < partitionCount; partition++) {
                            if (pairIndex >= pairGroups.length)
                                break;
                            ...
                            pair.forEach(({ seatId, studentName, gender }) => {
                                ...
                                if (student) {
                                    ...
                                    card.setAttribute('data-seat-id', seatId.toString());
                                    const seatNumberDiv = card.querySelector('.seat-number-label');
                                    if (seatNumberDiv) {
                                        seatNumberDiv.textContent = `#${seatId}`;
                                    }
                                    const nameDiv = card.querySelector('.student-name');
                    ...
                    // 카드 다시 가져오기
                    allCards = Array.from(seatsArea.querySelectorAll('.student-seat-card'));
                ...
                if (missingSeats.length > 0) {
                    ...
                    missingSeats.forEach(({ seatId, studentName }) => {
                        ...
                        let emptyCard = allCards.find(c => {
                            const nameDiv = c.querySelector('.student-name');
                            return nameDiv && !nameDiv.textContent?.trim();
                        });
                        if (emptyCard) {
                            emptyCard.setAttribute('data-seat-id', seatId.toString());
                            const seatNumberDiv = emptyCard.querySelector('.seat-number-label');
                            if (seatNumberDiv) {
                                seatNumberDiv.textContent = `#${seatId}`;
                            }
                            const nameDiv = emptyCard.querySelector('.student-name');
                            if (nameDiv) {
                                nameDiv.textContent = studentName;
                                restoredCount++;
                            }
    ...
    disableSidebarOptions() {
        ...
        const reverseGenderOrder = document.getElementById('reverse-gender-order');
        if (reverseGenderOrder) {
        ...
        const genderPairing = document.getElementById('gender-pairing');
        if (genderPairing) {
        ...
        // 옵션4: 맞춤 구성
        const customModeInputs = document.querySelectorAll('input[name="custom-mode-2"]');
        ...
        // 학생 이름 입력하기 버튼
        const createStudentTable = document.getElementById('create-student-table');
        ...
        const avoidPrevSeat = document.getElementById('avoid-prev-seat');
        if (avoidPrevSeat) {
    ...
    enableViewerMode(viewData) {
        try {
            ...
            // 비밀번호 확인
            if (shareInfo.p) {
                const passwordHash = shareInfo.p;
    ...
    showPrintView() {
        try {
            ...
            setTimeout(() => {
                ...
                const printLayout = classroomLayout.cloneNode(true);
                ...
                // 기존 그리드 레이아웃 가져오기
                const seatsArea = printLayout.querySelector('#seats-area');
        ...
        try {
            ...
            if (seatsArea) {
                ...
                if (seatCards.length > 0) {
                    ...
                    seatsAreaHeight = Math.max(maxBottom + 20, // 여유 공간 추가
                    seatsArea.scrollHeight, seatsArea.offsetHeight, seatsArea.getBoundingClientRect().height);
                }
                else {
                    // 카드가 없으면 기본 높이 사용
                    seatsAreaHeight = seatsArea.scrollHeight || seatsArea.offsetHeight || seatsArea.getBoundingClientRect().height;
                }
                seatsAreaWidth = Math.max(seatsArea.scrollWidth, seatsArea.offsetWidth, seatsArea.getBoundingClientRect().width);
            }
            ...
            const calculatedHeight = 20 + // 상단 패딩
                ...
                20; // 하단 패딩
            const totalHeight = Math.max(calculatedHeight, classroomLayout.scrollHeight, classroomLayout.offsetHeight, 80 + seatsAreaHeight, // 좌석 영역 시작 위치 + 높이
            rect.height);
            // 전체 너비 계산
            const totalWidth = Math.max(classroomLayout.scrollWidth, classroomLayout.offsetWidth, seatsAreaWidth + 40, // 좌석 영역 너비 + 좌우 패딩
            rect.width);
     ...
     */
    generateShareUrl(_seatsHtml, _gridColumns, _dateString, _expiresIn, _password) {
     ...
     * @deprecated 공유 기능이 제거되었습니다. 이 메서드는 더 이상 사용되지 않습니다.
     */
    async showShareModal(_shareUrl, _options) {
    ...
    showUserManual() {
        ...
        content.innerHTML = `
            <div style="line-height: 1.8; color: #444;">
                ...
                <ul style="padding-left: 25px; margin-bottom: 20px;">
                    <li><strong>✅ 자리 확정하기(저장) 💾</strong>: 자리 배치가 완료된 후 "✅ 자리 확정하기(저장) 💾" 버튼을 클릭하면 현재 배치가 해당 반의 이력에 저장되고 자동으로 저장됩니다</li>
                    <li><strong>⚠️ 중요</strong>: "자리 확정하기(저장) 💾" 버튼을 클릭하지 않으면 이력에 기록되지 않습니다</li>
     ...
     */
    checkAndHighlightClassCreation() {
    ...
    showInitialLayout() {
        try {
            ...
            const singleUniformRadio = document.querySelector('input[name="layout-type"][value="single-uniform"]');
            if (singleUniformRadio) {
                singleUniformRadio.checked = true;
                // 이벤트 발생 없이 설정 (자동 배치 방지)
            }
            // single-mode: 기본 1줄 배치 (basic-row)
            const basicRowRadio = document.querySelector('input[name="single-mode"][value="basic-row"]');
            if (basicRowRadio) {
     ...
     */
    async syncClassDataFromFirebase(classId) {
        if (!this.firebaseStorageManager.getIsAuthenticated()) {
            return;
        }
        try {

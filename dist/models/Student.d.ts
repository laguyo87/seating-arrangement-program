...
export declare class StudentModel {
     ...
     */
    static create(name: string, gender: 'M' | 'F'): Student;
    /**
     * 여러 학생을 한 번에 생성합니다.
     * @param studentsData 학생 데이터 배열 (이름과 성별)
     * @returns 생성된 Student 배열
     */
    static createMultiple(studentsData: Array<{
        name: string;
        gender: 'M' | 'F';
    }>): Student[];
    /**
     ...
     */
    static separateByGender(students: Student[]): {
        male: Student[];
        female: Student[];
    };
    /**
     ...
     */
    static getUnassignedStudents(students: Student[]): Student[];
    /**
     * 학생 목록에서 고정 좌석이 있는 학생만 필터링합니다.
     * @param students 학생 배열
     * @returns 고정 좌석이 있는 학생 배열
     */
    static getAssignedStudents(students: Student[]): Student[];
}

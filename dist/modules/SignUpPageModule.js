...
export class SignUpPageModule {
    ...
    setupEventListeners() {
        ...
        // 폼 제출
        const form = this.signUpPageContainer.querySelector('#signup-form');
        if (form) {
        ...
        // 비밀번호 확인 실시간 검증
        const passwordInput = this.signUpPageContainer.querySelector('#signup-password');
        const passwordConfirmInput = this.signUpPageContainer.querySelector('#signup-password-confirm');
    ...
    validatePasswordMatch() {
        ...
        const passwordInput = this.signUpPageContainer.querySelector('#signup-password');
        const passwordConfirmInput = this.signUpPageContainer.querySelector('#signup-password-confirm');
        const errorSpan = this.signUpPageContainer.querySelector('#signup-password-confirm-error');
        if (!passwordInput || !passwordConfirmInput || !errorSpan)
            return false;
    ...
    async handleSignUp() {
        if (!this.signUpPageContainer)
            return;
        const nameInput = this.signUpPageContainer.querySelector('#signup-name');
        const emailInput = this.signUpPageContainer.querySelector('#signup-email');
        const passwordInput = this.signUpPageContainer.querySelector('#signup-password');
        const passwordConfirmInput = this.signUpPageContainer.querySelector('#signup-password-confirm');
        const submitBtn = this.signUpPageContainer.querySelector('#signup-submit-btn');
        const statusDiv = this.signUpPageContainer.querySelector('#signup-page-status');
        if (!nameInput || !emailInput || !passwordInput || !passwordConfirmInput || !submitBtn || !statusDiv)
            return;
        // 비밀번호 일치 확인
        ...
        // 닉네임 검증
        if (name.length < 2) {
            const nameError = this.signUpPageContainer.querySelector('#signup-name-error');
            if (nameError) {
        ...
        if (!emailRegex.test(email)) {
            const emailError = this.signUpPageContainer.querySelector('#signup-email-error');
            if (emailError) {
        ...
        if (password.length < 6) {
            const passwordError = this.signUpPageContainer.querySelector('#signup-password-error');
            if (passwordError) {
        ...
        const emailError = this.signUpPageContainer.querySelector('#signup-email-error');
        const passwordError = this.signUpPageContainer.querySelector('#signup-password-error');
        const passwordConfirmError = this.signUpPageContainer.querySelector('#signup-password-confirm-error');
        if (nameError)
...
//# sourceMappingURL=SignUpPageModule.js.map
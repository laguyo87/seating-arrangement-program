...
export class InputService {
     ...
     * @returns 검증 결과
     */
    public static validateStudentCount(count: number): ValidationResult {
        if (count <= 0) {
     ...
     * @returns 검증 결과
     */
    public static validateStudentList(students: Student[], expectedCount: number): ValidationResult {
     ...
     * @returns 성별 분포 객체
     */
    public static calculateGenderRatio(students: Student[]): { male: number, female: number, total: number } {
     ...
     * @returns 균형잡혔는지 여부
     */
    public static isGenderRatioBalanced(students: Student[], maxDifference: number = 3): boolean {
     ...
     */
    public static createStudentFromInput(name: string, gender: string): Student | string {
     ...
     * @returns 가능 여부 및 메시지
     */
    public static canCreateGenderPairs(students: Student[]): { canPair: boolean, message: string } {
        const ratio = this.calculateGenderRatio(students);
